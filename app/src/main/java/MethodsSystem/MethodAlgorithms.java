...
public class MethodAlgorithms{
    ...
    public static int initBinarySearch(DataTable[] array, int dataIndex, Object target){
        ...
        if(type == DataTableType.TYPE_NULL){
            return -1;
        }
        switch(type){
            case TYPE_STRING -> {
    ...
    public static int binarySearch(DataTable[] array, int dataIndex, DataTableType type, int low, int high, Object target){
        ...
        switch(type){
            ...
            case TYPE_INTEGER -> {
                ...
                if(Objects.equals(value, (Integer) target)){
                    return mid;
                }
        ...
        switch(type){
            case TYPE_STRING -> {
                ...
                for(int i = start; i < end; i++){
                    ...
                    switch(order){
                        case Desc -> {
                            if(value.compareTo(pivotValueString) >= 0){
                                continue;
